"""V12: Music director cut II — richer piano + varied winds/strings rhythms + grand staff.

V11 → V12 changes:
  1. PIANO enrichment per phrase:
     - LH: octave bass / wide arpeggio (broken chord) per phrase
     - RH: 8-note close arpeggio / 16-note flowing arpeggio /
           chord+flourish / melody (mm.49-57 takes lead)
     - Piano grand staff (brace symbol on R.H. + L.H., shared barlines)
  2. WINDS variety:
     - Cl mm.17-24: 12-eighth obbligato → sparse call-and-response (rest + sustain)
     - Fl mm.41-48: 16-eighth obbligato → sparse fragment + rests
     - mm.55-57 build: 12-eighth ostinato → dramatic ascending dotted-halves
  3. STRINGS rhythmic variety:
     - Verses (mm.9-32): warm whole-bar Va/Vc sustain (mid-low rich) +
       Vn2 sparse 8th-motion (dotted half + dotted half)
     - Bridge 2 (mm.49-57): Piano takes melody, Vn1 sustained high pedal,
       all other strings warm sustained
     - Chorus (mm.66-74): Va/Vc sustained backdrop while Vn1 high octave shines
  4. Cb: pizz on beat 1 only (very sparse, anchor)
...
# ============================================================
# V12: Music director cut II
# 1) Piano enrichment: arpeggios, melody at bridge 2, lush voicings
# 2) Cl/Fl: replace continuous obbligato patterns with sparse phrases + rests
# 3) Strings: warm verse texture (Va/Vc sustained mid-low) + sparse Vn2 motion
# 4) mm.55-57 winds build: dramatic ascending with longer notes
# 5) mm.49-57: Piano takes melody, Vn1 swaps to sustained high pedal
# (Piano grand staff brace is applied during score assembly below)
# ============================================================

# ---- Piano LH/RH new patterns ----

def _v12_lh_octave_bass(chord_name):
    """Octave bass: root_lo+root_md (chord), 5th_md, root, 5th — 4 dotted quarters."""
    root_pc = CHORD_TONES_PC[chord_name]['root']
    fifth_pc = CHORD_TONES_PC[chord_name]['fifth']
    root_lo = f"{root_pc}1" if root_pc in {'A', 'B'} else f"{root_pc}2"
    root_md = f"{root_pc}2" if root_pc in {'A', 'B'} else f"{root_pc}3"
...
def _v12_lh_arpeggio(chord_name):
    """LH 8-note broken chord cycling root_lo - 5th_md - root_md (12 eighths)."""
    root_pc = CHORD_TONES_PC[chord_name]['root']
    fifth_pc = CHORD_TONES_PC[chord_name]['fifth']
    root_lo = f"{root_pc}1" if root_pc in {'A', 'B'} else f"{root_pc}2"
    root_md = f"{root_pc}2" if root_pc in {'A', 'B'} else f"{root_pc}3"
    ...
    out = []
    for _ in range(4):
        out.extend([N(root_lo, 0.5), N(fifth_md, 0.5), N(root_md, 0.5)])
    return out


def _v12_rh_arp_8th(chord_name):
    """RH 8-note close arpeggio: root - 3rd - 5th cycling (12 eighths in 12/8)."""
    voicing = CLOSE_VOICINGS_V10.get(chord_name, CLOSE_VOICINGS_V10['G'])
    pat = [voicing[0], voicing[1], voicing[2]]
    out = []
    for _ in range(4):
        for p in pat:
            out.append(N(p, 0.5))
...
def _v12_rh_arp_16th(chord_name):
    """RH 16-note flowing arpeggio (24 sixteenths) for lush flow."""
    voicing = CLOSE_VOICINGS_V10.get(chord_name, CLOSE_VOICINGS_V10['G'])
    # Pattern per dotted-quarter beat: root - 3rd - 5th - root_8va - 5th - 3rd (6 16ths)
    pat = [voicing[0], voicing[1], voicing[2], voicing[3], voicing[2], voicing[1]]
    out = []
    for _ in range(4):
        for p in pat:
            out.append(N(p, 0.25))
...
def _v12_rh_chord_with_flourish(chord_name):
    """Chord on beat 1+3 + 16-note flourish on beat 2+4 (chorus tutti)."""
    voicing = CLOSE_VOICINGS_V10.get(chord_name, CLOSE_VOICINGS_V10['G'])
    flourish_pat = [voicing[0], voicing[1], voicing[2], voicing[3], voicing[2], voicing[1]]
    out = []
    for beat in range(4):
        if beat % 2 == 0:
            out.append(CH(voicing, 1.5))
        else:
            for p in flourish_pat:
                out.append(N(p, 0.25))
    return out


def _v12_phrase_style(mn):
    """Return (lh_style, rh_style) per phrase."""
    if 9 <= mn < 17:    return ('octave', 'arp_8th')        # verse 1 light
    if 17 <= mn < 25:   return ('octave', 'arp_16th')       # verse 2 flowing
    if 25 <= mn < 33:   return ('octave', 'arp_8th')        # verse 3
    if 33 <= mn < 41:   return ('octave', 'arp_16th')       # bridge cantabile lush flow
    if 41 <= mn < 49:   return ('arp', 'arp_16th')          # pre-chorus build
    if 49 <= mn < 58:   return ('arp', 'melody')            # bridge 2 — Piano takes melody
    if 58 <= mn < 66:   return ('octave', 'arp_16th')       # climax
    if 66 <= mn < 75:   return ('octave', 'chord_flourish') # chorus tutti
    return ('octave', 'arp_8th')


# Apply Piano overrides for mm.9-74 (intro/outro keep V5 ostinato)
for _mn in range(9, 75):
    _ch = detect_chord(_mn) or 'G'
    _lh, _rh = _v12_phrase_style(_mn)

    # LH
    if _lh == 'octave':
        pno_lh_data[_mn] = normalize(_v12_lh_octave_bass(_ch))
    elif _lh == 'arp':
        pno_lh_data[_mn] = normalize(_v12_lh_arpeggio(_ch))

    # RH
    if _rh == 'arp_8th':
        pno_rh_data[_mn] = normalize(_v12_rh_arp_8th(_ch))
    elif _rh == 'arp_16th':
        pno_rh_data[_mn] = normalize(_v12_rh_arp_16th(_ch))
    elif _rh == 'chord_flourish':
        pno_rh_data[_mn] = normalize(_v12_rh_chord_with_flourish(_ch))
    elif _rh == 'melody':
        # Piano RH takes the original Voice/Oohs melody, fitted to comfortable C4-C6
        _voice = [clone(n) for _, n in measure_events(P_VOX, _mn)]
        if _voice:
            _voice = shift_to_range(_voice, 60, 84)
            pno_rh_data[_mn] = normalize(_voice)


# ---- Vn1 swap: mm.49-57 sustained high pedal (Piano takes melody) ----
for _mn in range(49, 58):
    _ch = detect_chord(_mn) or 'G'
    _f_pc = CHORD_TONES_PC[_ch]['fifth']
    # Sustained high pedal on chord 5th
    _f5 = f"{_f_pc}5"
    vn1_data[_mn] = [N(_f5, BAR_QL)]


# ---- Cl mm.17-24: replace 12-eighth obbligato with sparse call-and-response ----
def _cl_v12_phrase(chord_name, pos):
    t_pc = CHORD_TONES_PC[chord_name]['third']
    f_pc = CHORD_TONES_PC[chord_name]['fifth']
    if pos == 0:
        # Brief ascending response in last beat
        return [R(4.5), N(f"{t_pc}5", 0.5), N(f"{f_pc}5", 0.5), N(f"{t_pc}5", 0.5)]
    if pos == 1:
        # Rest
        return [R(BAR_QL)]
    if pos == 2:
        # Sustained color note (long)
        return [N(f"{f_pc}5", 3.0), R(3.0)]
    if pos == 3:
        return [R(BAR_QL)]
    return [R(BAR_QL)]


for _i, _mn in enumerate(range(17, 25)):
    _ch = detect_chord(_mn) or 'G'
    cl_data[_mn] = normalize(_cl_v12_phrase(_ch, _i % 4))


# ---- Fl mm.41-48: replace 16-eighth obbligato with sparse fragment ----
def _fl_v12_phrase(chord_name, pos):
    t_pc = CHORD_TONES_PC[chord_name]['third']
    f_pc = CHORD_TONES_PC[chord_name]['fifth']
    r_pc = CHORD_TONES_PC[chord_name]['root']
    if pos == 0:
        return [R(BAR_QL)]
    if pos == 1:
        # Brief descending fragment
        return [R(3.0), N(f"{r_pc}6", 0.5), N(f"{f_pc}5", 0.5), N(f"{t_pc}5", 1.0), R(1.0)]
    if pos == 2:
        return [N(f"{f_pc}5", 3.0), R(3.0)]
    if pos == 3:
        return [R(BAR_QL)]
...
for _i, _mn in enumerate(range(41, 49)):
    _ch = detect_chord(_mn) or 'G'
    fl_data[_mn] = normalize(_fl_v12_phrase(_ch, _i % 4))


# ---- Winds mm.55-57: dramatic ascending build with longer notes ----
for _mn in [55, 56, 57]:
    _ch = detect_chord(_mn) or 'G'
    _r = CHORD_TONES_PC[_ch]['root']
    _t = CHORD_TONES_PC[_ch]['third']
    _f = CHORD_TONES_PC[_ch]['fifth']
    if _mn == 55:
        # Cl: ascending dotted halves
        cl_data[_mn] = normalize([N(f"{_r}5", 1.5), N(f"{_t}5", 1.5), N(f"{_f}5", 3.0)])
        # Fl: 1-bar rest then enter
        fl_data[_mn] = [R(BAR_QL)]
    elif _mn == 56:
        cl_data[_mn] = normalize([N(f"{_r}5", 3.0), N(f"{_t}5", 3.0)])
        fl_data[_mn] = normalize([N(f"{_t}5", 1.5), N(f"{_f}5", 1.5),
                                  N(f"{_r}6", 1.5), N(f"{_t}5", 1.5)])
    elif _mn == 57:
        # Sustain into climax
        cl_data[_mn] = normalize([N(f"{_f}5", BAR_QL)])
        fl_data[_mn] = normalize([N(f"{_f}5", BAR_QL)])


# ---- Strings: replace V8 4-dotted-quarter pulse with phrase-specific patterns ----
# Verses (mm.9-32): Va/Vc warm whole-bar sustain (mid-low rich), Vn2 sparse 8th-motion
# Bridge 2 (mm.49-57): all strings sustained (Piano is melody)
# Chorus (mm.66-74): Vn1 high octave-doubled (V11 already), Vn2 high harmony, Va/Vc sustained mid

def _vn2_v12_sparse_motion(chord_name):
    """Sparse motion: dotted half + dotted half on chord tones (5-3 alternation)."""
    f_pc = CHORD_TONES_PC[chord_name]['fifth']
    t_pc = CHORD_TONES_PC[chord_name]['third']
    f4 = _fit_oct(f_pc, 55, 76, (4, 5, 3))
    t4 = _fit_oct(t_pc, 55, 76, (4, 5, 3))
    return [N(f4, 3.0), N(t4, 3.0)]


def _va_v12_warm(chord_name):
    """Va warm whole-bar sustain on chord 3rd (viola sweet spot)."""
    t_pc = CHORD_TONES_PC[chord_name]['third']
    return [N(_fit_oct(t_pc, 48, 64, (3, 4)), BAR_QL)]


def _vc_v12_warm(chord_name):
    """Vc warm whole-bar sustain on chord root (cello sweet spot)."""
    r_pc = CHORD_TONES_PC[chord_name]['root']
    return [N(_fit_oct(r_pc, 36, 55, (2, 3)), BAR_QL)]


def _cb_v12_pizz1(chord_name):
    """Cb pizz on beat 1 only (sparse anchor)."""
    r_pc = CHORD_TONES_PC[chord_name]['root']
    return [N(_fit_oct(r_pc, 40, 55, (2, 3)), 1.5), R(4.5)]


# Verse phrases (mm.9-32): warm sustained
for _mn in range(9, 33):
    _ch = detect_chord(_mn) or 'G'
    _plan = MELODY_PLAN.get(_mn, {})
    if 'Vn2' not in _plan and 'Vn2_lower6' not in _plan:
        vn2_data[_mn] = _vn2_v12_sparse_motion(_ch)
    if 'Va' not in _plan:
        va_data[_mn] = _va_v12_warm(_ch)
    if 'Vc' not in _plan:
        vc_data[_mn] = _vc_v12_warm(_ch)
    cb_data[_mn] = _cb_v12_pizz1(_ch)

# Bridge 2 (mm.49-57): Piano is melody, all strings sustained
for _mn in range(49, 58):
    _ch = detect_chord(_mn) or 'G'
    # Vn1 already overridden above (sustained high pedal)
    vn2_data[_mn] = [N(_fit_oct(CHORD_TONES_PC[_ch]['fifth'], 55, 76, (4, 5, 3)), BAR_QL)]
    va_data[_mn] = _va_v12_warm(_ch)
    vc_data[_mn] = _vc_v12_warm(_ch)
    cb_data[_mn] = _cb_v12_pizz1(_ch)

# Chorus (mm.66-74): Vn2 high harmony already from V9 (diatonic 6th below).
# Va/Vc/Cb: sustained mid-low for backdrop
for _mn in range(66, 75):
    _ch = detect_chord(_mn) or 'G'
    _plan = MELODY_PLAN.get(_mn, {})
    if 'Va' not in _plan:
    ...
    if 'Vc' not in _plan:
        vc_data[_mn] = _vc_v12_warm(_ch)
    # Cb stays rhythmic for chorus (V8 already)
...
score = make_score(
    ...
    subtitle=TASK["output"]["subtitle"] + " (V12 — Director's Cut II)",
    composer="(original)",
    arranger="arr. for 9-part orchestra (V12)",
    parts=[fl, cl, vn1, vn2, va, vc, cb, pno_rh, pno_lh, drm],
    group_label="Orchestra",
    group_symbol="bracket",
)

# V12: Piano grand staff — bind Piano R.H. + L.H. with brace, shared barlines
from music21 import layout as _layoutmod
_piano_brace = _layoutmod.StaffGroup([pno_rh, pno_lh], name='Piano',
                                       symbol='brace', barTogether=True)
score.insert(0, _piano_brace)
