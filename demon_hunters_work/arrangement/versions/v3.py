"""V3: demon_hunters -> 9-part orchestra (music director cut, professional players).

Conductor's principles:
  1. ROLE SEPARATION — each instrument has ONE role per phrase (no mixed duties)
  2. MELODY CLARITY — solo melody in upper register; pad in mid-low; piano avoids melody octave
  3. PIANO REWRITTEN — discarded OMR raw RH/LH; new playable voicings (LH bass pulse, RH close-position chords)
  4. ORCHESTRAL IDIOM — pizzicato, tremolo, divisi, double-stops, harmonics, octave doublings
  5. DYNAMIC ARC — verse(thin) → bridge → climax(layered) → chorus(tutti 4-part harmony) → outro(decay)
...
# ============================================================
# Piano (REWRITTEN — playable, two-handed)
# ============================================================
# LH: bass pulse — root, fifth, root, fifth (4 dotted quarters)
def piano_lh_bar(chord_name, dyn='normal'):
    if not chord_name:
        return [R(BAR_QL)]
    root_pc = CHORD_TONES_PC[chord_name]['root']
    fifth_pc = CHORD_TONES_PC[chord_name]['fifth']
    # Bass register: A1 (33) to G3 (55). Place root in oct 2, fifth in oct 2 or 3.
    root = f"{root_pc}2"
    # Fifth: avoid going below A1; if fifth pc is C/D/E (low), use oct 3
    fifth_lower_pcs = {'C', 'D', 'E', 'F', 'F#'}
    fifth_oct = 3 if fifth_pc in fifth_lower_pcs else 2
    fifth = f"{fifth_pc}{fifth_oct}"
    if dyn == 'sparse':
        return [N(root, 1.5), R(1.5), N(root, 1.5), R(1.5)]
    if dyn == 'verse':
        return [N(root, 1.5), N(fifth, 1.5), N(root, 1.5), N(fifth, 1.5)]
    if dyn == 'full':
        # arpeggiated: root - 5th - 8va - 5th
        root_8va = f"{root_pc}3"
        return [N(root, 0.75), N(fifth, 0.75), N(root_8va, 0.75), N(fifth, 0.75)] * 2
    return [N(root, 1.5), N(fifth, 1.5), N(root, 1.5), N(fifth, 1.5)]


# RH: close-position triad voicing in C4-G5 register, NO melody-doubling
def piano_rh_bar(chord_name, dyn='normal'):
    ...
    fifth_pc = CHORD_TONES_PC[chord_name]['fifth']
    # Build 3-note close voicing in C4-A4 region for harmonic cushion
    # Voicing: 3rd-5th-root(8va) typical close
    third = f"{third_pc}4"
    fifth = f"{fifth_pc}4"
    # Determine root octave so it sits above third (close voicing)
    # If root pc < third pc (C below E etc.), put root in oct 5
    pc_order = {'C': 0, 'D': 2, 'E': 4, 'F': 5, 'F#': 6, 'G': 7, 'A': 9, 'B': 11}
    root_pc_no_acc = root_pc.replace('#', '').replace('b', '')
    third_pc_no_acc = third_pc.replace('#', '').replace('b', '')
    root_pos = pc_order.get(root_pc, 0)
    third_pos = pc_order.get(third_pc, 0)
    root_oct = 5 if root_pos < third_pos else 4
    root = f"{root_pc}{root_oct}"
    voicing = [third, fifth, root]   # close-position 3-note chord

    if dyn == 'sparse':
        # chord on beat 1 only
        return [CH(voicing, 1.5), R(BAR_QL - 1.5)]
    if dyn == 'verse':
        # chord on beats 1 and 3
        return [CH(voicing, 1.5), R(1.5), CH(voicing, 1.5), R(1.5)]
    if dyn == 'full':
        # chord every beat
        return [CH(voicing, 1.5) for _ in range(4)]
    if dyn == 'arpeggio':
        # broken chord: 3rd-5th-root-5th per beat (eighths)
        return [N(third, 0.5), N(fifth, 0.5), N(root, 0.5)] * 4
    # normal: chord every other beat (1, 3) with rest on 2, 4
    return [CH(voicing, 1.5), R(1.5), CH(voicing, 1.5), R(1.5)]
